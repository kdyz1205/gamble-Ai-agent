...
**Core rule.** When the user describes a goal, execute it fully, autonomously, rigorously, to the highest spec — without asking the user to click, paste, verify, or do any manual work Codex could do itself. Do hard things thoroughly; do NOT take the easy shortcut just because it's available.
...
- Make a decision Codex has the information to make
...
Codex should behave as a disciplined product engineer inside this repo.
...
The product direction is a bright, playful, Pokemon GO-like world for real-world challenges.

Users are Summoners.
AI helpers are Familiars.
Challenges are Quests.
Groups and communities are Arenas.
Completed results become shareable Receipts.

The product is not an AI betting agent, sportsbook, crypto casino, public prediction market, or dark gambling cockpit.

Prioritize:
- chat-first challenge creation
- quest/proof/familiar language
- mobile-first UX
- bright map-world visual style
- proof-based AI judgment
- shareable result receipts
- internal-credit / non-cash first positioning
...
Default Codex rule:
Make small, reviewable, presentational changes unless explicitly asked to touch backend logic. Do not modify schema, state machine, API contracts, credits, settlement, evidence, payment, auth, or judge backend behavior just to match product language. See `docs/summoner-product-direction.md` for the fuller direction.
