...
async function main() {
  ...
  const examples = [
    ...
    example("Will Tesla file an 8-K before the market opens tomorrow?", "sec_edgar_submissions", false),
    example("Will the FDA announce a Class I food recall involving peanut butter this month?", "openfda_food_enforcement", false),
    example("Will repository vercel/next.js publish a GitHub release before Friday?", "github_releases", false),
    example("Will next publish version of npm package zod be at least 4.2.0 by Friday?", "npm_registry_package", false),
    example("Will pandas publish a new PyPI release before next Monday?", "pypi_json", false),
    example("Will Docker Hub publish a python image tag named 3.13-alpine this week?", "dockerhub_tags", false),
    example("Will invoice in_123 be marked paid before 5 PM?", "stripe_invoice", false),
    example("Will domain example.com publish a TXT record containing verify-axelrod by midnight?", "cloudflare_dns_over_https", false),
  ];
